...
13. 문제 풀이 화면에서 주판 입력값을 표시하고, OK 신호가 들어오면 정답을 검사한다.
...
| 확인 신호 후보 | `data[data.length - 1]` | OK 버튼 또는 확인 신호 |
...
- OK 버튼이 항상 마지막 바이트 `0x01`인지 실기 테스트로 확인한다.
- OK 버튼을 누른 뒤 release packet이 따로 오는지 확인한다.
- 십의 자리/일의 자리 외에 백의 자리 이상을 지원하는지 확인한다.
- 음수 입력, 초기화 버튼, 지우기 버튼이 별도 신호로 존재하는지 확인한다.
...
- raw hex 디버그 표시
- OK 신호 수신 시 `isConfirmed` true 처리
- 연결 실패/해제 상태 메시지 표시
- 더미 연결 및 더미 숫자 입력 유지
...
- OK 버튼을 누르면 현재 숫자가 정답 검사 함수로 전달된다.
...
interface InputController {
  ...
  clearAnswer(): void;
  submitAnswer(source: AnswerSource): void;
}
```
...
- `submitAnswer(source)`는 현재 입력값을 문제 풀이 로직에 제출한다.
...
- 블루투스 확인 입력은 `setAnswer(value)` 후 `submitAnswer("bluetooth")`를 실행한다.
...
function handleBluetoothConfirm(event: BluetoothAnswerEvent) {
  ...
  inputController.submitAnswer("bluetooth");
}
...
- `submitAnswer`는 현재 문제 상태가 입력 가능할 때만 동작한다.
- `submitAnswer`는 제출 시작 시 `isSubmitting`을 `true`로 바꾼다.
- 제출이 끝나고 다음 문제로 넘어가기 전까지 추가 제출을 막는다.
- 이미 제출된 문제라면 이후 들어온 블루투스 확인 신호와 키패드 제출을 모두 무시한다.
- 다음 문제로 넘어갈 때 `answer`, `isSubmitting`, `submittedQuestionIndex`를 초기화한다.
- `submittedQuestionIndex`가 현재 문제 번호와 같으면 같은 문제의 추가 제출을 무시한다.
...
- `lastData.isConfirmed === true`가 되면 `InputController`에 현재 입력값을 전달한다.
- 블루투스 확인 입력은 `setAnswer(String(lastData.number))` 후 `submitAnswer("bluetooth")`를 실행한다.
- 같은 OK packet이 여러 번 처리되지 않도록 debounce 또는 processed flag를 둔다.
...
- TODO: OK 버튼 raw 값과 release raw 값을 분리해서 확인한다.
