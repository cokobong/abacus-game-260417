...
- 기본 입력은 Bluetooth 주판이다.
- 숫자패드는 Bluetooth 주판이 없거나 사용할 수 없을 때를 위한 보조 입력 수단이다.
- Bluetooth 입력과 숫자패드 입력은 같은 answer input 상태를 공유한다.
- 숫자패드의 `입력` 버튼과 Bluetooth 리턴/확인 버튼은 같은 정답 제출 로직을 호출한다.
- 숫자패드는 보조 수단이므로 훈련장 문제 카드보다 작게, compact하게 배치한다.
- 최종 시각 디자인은 전체 UI 리디자인 단계에서 다시 맞춘다.
