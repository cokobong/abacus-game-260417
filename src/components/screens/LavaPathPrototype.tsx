...
export const LAVA_VALLEY_DIFFICULTY = {
  easy: { playerVisualScale: 1.28, obstacleVisualScale: 1.2, runSpeed: 26.5, obstacleSpawnIntervalMin: 3100, obstacleSpawnIntervalMax: 4400, jumpVelocity: 101, gravity: 210, playerHitboxScale: .68, obstacleHitboxScale: .72, coyoteTimeMs: 180, jumpBufferMs: 250, jumpGuideEnabled: true, geyserChance: .18 },
  normal: { playerVisualScale: 1.25, obstacleVisualScale: 1.18, runSpeed: 32, obstacleSpawnIntervalMin: 2400, obstacleSpawnIntervalMax: 3600, jumpVelocity: 98, gravity: 230, playerHitboxScale: .76, obstacleHitboxScale: .78, coyoteTimeMs: 110, jumpBufferMs: 120, jumpGuideEnabled: false, geyserChance: .35 },
  challenge: { playerVisualScale: 1.25, obstacleVisualScale: 1.18, runSpeed: 36, obstacleSpawnIntervalMin: 2100, obstacleSpawnIntervalMax: 3100, jumpVelocity: 96, gravity: 240, playerHitboxScale: .84, obstacleHitboxScale: .88, coyoteTimeMs: 70, jumpBufferMs: 70, jumpGuideEnabled: false, geyserChance: .5 },
} as const;
...
export function LavaPathPrototype({ dinosaur, onExit }: LavaPathPrototypeProps) {
  ...
  const [jumpGuideActive, setJumpGuideActive] = useState(false);
  ...
  const jumpYRef = useRef(0), jumpVelocityRef = useRef(0), difficultyRef = useRef<Difficulty>('easy'), lastGroundedAtRef = useRef(performance.now()), jumpBufferedUntilRef = useRef(0), hasJumpedSinceGroundRef = useRef(false);
  const guidedObstacleIdsRef = useRef(new Set<number>()), jumpGuideTimerRef = useRef<number | null>(null);
  ...
  const chooseDifficulty = (next: Difficulty) => { difficultyRef.current = next; setDifficulty(next); setJumpGuideActive(false); const preset = LAVA_VALLEY_DIFFICULTY[next]; nextObstacleRef.current = pauseStartedRef.current + randomBetween(preset.obstacleSpawnIntervalMin, preset.obstacleSpawnIntervalMax); };
  const finish = (next: Result) => { resultRef.current = next; itemsRef.current = []; setItems([]); setResult(next); };
  const reset = useCallback(() => { itemsRef.current = []; healthRef.current = 3; resultRef.current = 'playing'; jumpingRef.current = false; jumpYRef.current = 0; jumpVelocityRef.current = 0; invincibleUntilRef.current = 0; pausedRef.current = false; jumpBufferedUntilRef.current = 0; hasJumpedSinceGroundRef.current = false; guidedObstacleIdsRef.current.clear(); checkpointSpawnedRef.current = false; checkpointPassedRef.current = false; startTimeRef.current = performance.now(); lastFrameRef.current = 0; nextObstacleRef.current = 0; nextCollectibleRef.current = 0; setItems([]); setCoins(0); setRareShards(0); setHealth(3); setTimeLeft(LAVA_RUNNER_CONFIG.gameDuration); setJumping(false); setJumpY(0); setInvincible(false); setPaused(false); setResult('playing'); setCheckpointStage(1); setPickupFeedback(null); setJumpGuideActive(false); setRunId((value) => value + 1); }, []);
  ...
  useEffect(() => () => { if (jumpGuideTimerRef.current) window.clearTimeout(jumpGuideTimerRef.current); }, []);
  useEffect(() => {
    let frame = 0;
    ...
    const tick = (now: number) => {
      ...
      if (preset.jumpGuideEnabled) { const guideObstacle = itemsRef.current.find((item) => isObstacle(item.kind) && item.x > LAVA_RUNNER_CONFIG.playerX && item.x <= LAVA_RUNNER_CONFIG.playerX + 34 && !guidedObstacleIdsRef.current.has(item.id)); if (guideObstacle) { guidedObstacleIdsRef.current.add(guideObstacle.id); setJumpGuideActive(false); requestAnimationFrame(() => setJumpGuideActive(true)); if (jumpGuideTimerRef.current) window.clearTimeout(jumpGuideTimerRef.current); jumpGuideTimerRef.current = window.setTimeout(() => setJumpGuideActive(false), 650); } }
  ...
  return <section className={`lava-runner ${paused ? 'lava-runner--paused' : ''}`} style={{ '--lava-player-x': `${LAVA_RUNNER_CONFIG.playerX}%`, '--lava-track-bottom': `${LAVA_RUNNER_CONFIG.trackBottom}%`, '--lava-track-duration': `${2.8 * 32 / LAVA_VALLEY_DIFFICULTY[difficulty].runSpeed}s`, '--lava-player-scale': LAVA_VALLEY_DIFFICULTY[difficulty].playerVisualScale, '--lava-obstacle-scale': LAVA_VALLEY_DIFFICULTY[difficulty].obstacleVisualScale } as CSSProperties}>
    ...
    <footer className="lava-runner__controls"><button type="button" className="lava-runner__dash" disabled>대시<small>준비 중</small></button><button type="button" className={`lava-runner__jump ${jumpGuideActive ? 'lava-runner__jump--guide' : ''}`} onPointerDown={(event) => { event.preventDefault(); jump(); }} aria-label="점프"><span>점프! <b>⬆</b></span></button></footer>
