...
function regionStages(regionId: AdventureRegionId, names: readonly string[], descriptions: readonly string[]): AdventureStage[] {
  return ([1, 2, 3] as const).map((stageNumber, index) => ({
    ...
    implemented: regionId === 'lavaValley' || regionId === 'skyIsland' || regionId === 'ancientRuins' || regionId === 'deepSeaCanyon' || (regionId === 'iceContinent' && stageNumber <= 2),
...
export const ADVENTURE_STAGE_CATALOG: Record<AdventureRegionId, AdventureStage[]> = {
  ...
  iceContinent: regionStages('iceContinent', ['빙하 연구소', '얼음 광산', '고대 빙하 발전소'], ['이동과 수리 방법을 배우고 복원 기지를 가동해요', '소형 화석 4종을 복원하며 공장을 운영해요', '원인과 결과 체인 (준비 중)']),
};
