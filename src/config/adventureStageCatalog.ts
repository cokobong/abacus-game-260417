...
function regionStages(regionId: AdventureRegionId, names: readonly string[], descriptions: readonly string[]): AdventureStage[] {
  return ([1, 2, 3] as const).map((stageNumber, index) => ({
    ...
    implemented: regionId === 'lavaValley' || regionId === 'skyIsland' || (regionId === 'ancientRuins' && stageNumber <= 2) || (regionId === 'deepSeaCanyon' && stageNumber <= 2),
...
export function getRegionForGame(gameId: string): AdventureRegionId | undefined {
  return ({ 'lava-stepping-stones': 'lavaValley', 'sky-number-clouds': 'skyIsland', 'number-ruins': 'ancientRuins', 'deep-sea-explorer': 'deepSeaCanyon' } as const)[gameId];
}
