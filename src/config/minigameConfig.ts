...
export type MinigameId = 'lava-stepping-stones' | 'sky-number-clouds' | 'number-ruins' | 'deep-sea-explorer' | 'ice-operation-arcade';
...
export function chargeMinigameEntry(coins: number, gameId: MinigameId, stage: AdventureStageNumber = 1, retryAfterFailure = false) {
  const regionId = gameId === 'lava-stepping-stones' ? 'lavaValley' : gameId === 'sky-number-clouds' ? 'skyIsland' : gameId === 'deep-sea-explorer' ? 'deepSeaCanyon' : gameId === 'ice-operation-arcade' ? 'iceContinent' : 'ancientRuins';
  const configuredCost = getAdventureRunCost(regionId, stage, retryAfterFailure);
  const cost = gameId === 'number-ruins' || gameId === 'deep-sea-explorer' || gameId === 'ice-operation-arcade' ? MINIGAME_ENTRY_COST[gameId] : configuredCost;
